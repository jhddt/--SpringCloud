    ...
    <el-dialog v-model="sendDialogVisible" title="发送消息" width="600px">
      <el-form :model="sendForm" label-width="100px" :rules="sendRules" ref="sendFormRef">
        <el-form-item label="消息类型" prop="messageType" required>
          <el-tag type="info">即时消息</el-tag>
          <span style="margin-left: 10px; color: #909399; font-size: 12px;">（学生只能发送私聊消息）</span>
        </el-form-item>
        <el-form-item label="范围类型" prop="scopeType" required>
          <el-tag type="info">私聊</el-tag>
        </el-form-item>
...
// 会话侧边栏
const chatConversations = ref([]) // [{ key, userId, type, name, lastTime, lastContent }]
...
const loadStudentList = async () => {
  try {
    ...
    if (response.data.code === 200) {
      // 过滤掉当前用户
      studentList.value = (response.data.data.records || []).filter(
        s => (s.studentId !== userStore.studentId) && (s.userId !== userStore.userId)
      )
    }
  } catch (error) {
  ...
  await sendFormRef.value.validate(async (valid) => {
    ...
    try {
      ...
      const sendData = {
        ...
        receiverType: sendForm.value.receiverType
      }
